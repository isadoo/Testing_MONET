...
#SBATCH --job-name=monet_logav_only
#SBATCH --output=logav_only_logs/logav_%A_%a.stdout
#SBATCH --error=logav_only_logs/logav_%A_%a.stderr
#SBATCH --mem=16G
...
# Run monet: logAV-only analysis (no environmental covariates)
# Only Quercus faginea was studied in both common gardens;
# Q. lusitanica was studied in the greenhouse only.
# Both faginea gardens share the same wild-population genetic data,
# so the same ThetaP is reused for faginea OC.
#
#   0: faginea    GH
#   1: faginea    OC
#   2: lusitanica GH
...
export MONET_OUTDIR="$CASE_DIR/results_logAV_only"
