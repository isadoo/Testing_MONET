...
# Read run3 data (TSV) - 3x3 sires/dams with new SS
run3 <- read.delim("/work/FAC/FBM/DEE/jgoudet/default/isaChapter2/isaChapter2/Testing_simulated_data/results_tables/run3_combined_newSS.tsv", 
                   header = TRUE, sep = "\t", stringsAsFactors = FALSE)

# Read run4 data (TSV) - 1x9 sires/dams with new SS (no QST-FST)
run4 <- read.delim("/work/FAC/FBM/DEE/jgoudet/default/isaChapter2/isaChapter2/Testing_simulated_data/results_tables/run4_combined_newSS.tsv", 
                   header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
cat("Rows in all_runs_filtered:", nrow(all_runs_filtered), "\n\n")

# Add sire_dam_design column
run3$sire_dam_design <- "3x3"
run4$sire_dam_design <- "1x9"
all_runs_filtered$sire_dam_design <- "5x5"

# Add placeholder columns for run4 if QST-FST is missing
if (!"p_value_QSTFST" %in% colnames(run4)) {
  cat("Adding placeholder QST-FST columns to run4\n")
  run4$p_value_QSTFST <- NA
  run4$p_value_QSTFST_Neutral <- NA
}

# Combine datasets
df <- rbind(run3, run4, all_runs_filtered)
...
for (model_type in c("IM", "SS", "hierarchical")) {
  ...
  if (nrow(subset_df) > 0) {
    print(table(subset_df$correlation, subset_df$sire_dam_design, useNA = "always"))
  }
}
...
process_scenario <- function(scenario_data, scenario_name) {
  ...
  # QSTFST - check if data is available (not all NA)
  if (sum(!is.na(scenario_data$p_value_QSTFST)) > 0) {
    ...
    cat("  Skipping QSTFST - no data available\n")
...
## ------------------ DEFINE SCENARIOS ------------------
scenarios <- list(
  list(name = "Island Model", model = "IM", correlation = NA, sire_dam_design = c("3x3", "1x9", "5x5")),
  list(name = "SS - Cline", model = "SS", correlation = "_cline", sire_dam_design = c("3x3", "1x9", "5x5")),
  list(name = "SS - Sine", model = "SS", correlation = "_sine", sire_dam_design = c("3x3", "1x9", "5x5")),
  list(name = "Hierarchical - Grouped", model = "hierarchical", correlation = "_grouped", sire_dam_design = c("3x3", "1x9", "5x5")),
  list(name = "Hierarchical - Swapped", model = "hierarchical", correlation = "_swapped", sire_dam_design = c("3x3", "1x9", "5x5"))
)
...
for (i in 1:length(scenarios)) {
  ...
  for (design in scenario$sire_dam_design) {
    ...
    if (!is.null(result)) {
      ...
      result$roc_data$sire_dam_design <- design
      result$roc_data$graph_name <- scenario$name
      result$auc_data$graph_id <- i
      result$auc_data$sire_dam_design <- design
...
cat("\n=== DATA SUMMARY ===\n")
cat("Total ROC points:", nrow(all_roc_data), "\n")
cat("Sire/Dam designs present:\n")
print(table(all_roc_data$sire_dam_design))
cat("\nMethods present:\n")
print(table(all_roc_data$method))
cat("\nScenarios present:\n")
print(table(all_roc_data$graph_name))
cat("\nBy design and method:\n")
print(table(all_roc_data$sire_dam_design, all_roc_data$method))

## ------------------ PLOTTING ------------------
agg_png("results_December2025/ROC_SireDamNumber_Comparison_newSS.png",
...
par(mar = c(0, 0, 1, 0))
plot.new()
text(0.5, 0.5, "Breeding tests: Dam x Sire number", cex = 3.5, font = 2)
...
for (graph_id in 1:5) {
  ...
  cat("  Available designs:", unique(plot_data$sire_dam_design), "\n")
  ...
  for (design in c("3x3", "1x9", "5x5")) {
    tmp_design <- plot_data[plot_data$sire_dam_design == design, ]
    
    if (nrow(tmp_design) == 0) {
      cat("  Warning: No data for", design, "\n")
      next
    }
    ...
    for (method in c("Driftsel", "QSTFST", "MONET")) {
      ...
      if (nrow(tmp_method) == 0) {
        cat("  Warning: No data for", method, "with", design, "\n")
        next
      }
      ...
      cat("  Plotting:", method, "with", design, "(", nrow(tmp_method), "points)",
...
write.csv(all_auc_data, "results_December2025/AUC_SireDamNumber_summary_newSS.csv", row.names = FALSE)
