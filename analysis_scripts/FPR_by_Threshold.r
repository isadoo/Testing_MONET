...
df_main <- read.csv("/home/isa/github_repositories/Testing_LAVA/raw_data/3methods_full_breeding_combined_november.tsv", header = TRUE, sep = "\t")
...
process_population_structure <- function(pop_data, pop_name) {
  ...
  for (i in 1:nrow(scenarios)) {
    ...
    roc_lava <- prepare_for_linear_plot(roc_lava)
    ...
    roc_habitat$method <- "LAVA w/ environment"
    ...
    roc_data_list[[scenario_name]] <- rbind(roc_qstfst, roc_lava, roc_driftsel, roc_habitat)
  }
  ...
  legend("center",
         legend = c("Driftsel", "QSTFST", "LAVA", "LAVA w/ environment", paste0(omega_chr, " = ", c(10, 22, 50))),
         ncol   = 2,
